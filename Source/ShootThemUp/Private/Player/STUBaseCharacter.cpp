...
// ReSharper disable once CppPrecompiledHeaderNotFound
...
{
    const float FallVellocityZ = -GetVelocity().Z;
    UE_LOG(BaseCharacterLog, Display, TEXT("Fall Vellocity Z : %f"), FallVellocityZ);

    if (FallVellocityZ < LandedDamageVelocity.X) return;

    const auto FinalDamage = FMath::GetMappedRangeValueClamped(LandedDamageVelocity, LandDamage, FallVellocityZ);
